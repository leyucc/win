...
class FaceMakeupSingle(SingleTask):
    '''人脸美妆单个'''
    ...
    def work(self, templeUrl, ResourceType, Strength):
        try:
            ...
            url = self.aliFace.FaceMakeupRequest(url2, ResourceType=ResourceType, Strength=Strength)
    ...
    def btnStartClicked(self):
        ...
        if len(items1) <= 0:
            self.log('请先选中一个模板')
            return
        items0 = self.listSource.selectedIndexes()
        if len(items1) <= 0:
            self.log('请先选中一个滤镜类型')
            return
        templeUrl = items1[0].text()
        ResourceType = str(items0[0].row())
        Strength = float(self.main.horizontalSlider_3.value() * 0.01)
...
class FaceMakeupMulti(MultiTask):
    '''人脸美妆批量'''
    ...
    def work(self):
        ...
        qli: QListWidget = self.main.listSource_9
        ResourceType = str(qli.currentIndex().row())
        Strength = float(self.main.horizontalSlider_4.value() * 0.01)
        while self.state:
            temples = []
